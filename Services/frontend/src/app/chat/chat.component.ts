...
export class ChatComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  ngOnInit(): void {
    this.auth.getProfile().subscribe({
      next: (profile) => {
        ...
        if (!profile || !profile.friend_email || !profile.friend_phone) {
          console.warn('[CHAT] Emergency contact info is incomplete! Redirecting to setup wizard.');
    ...
    this.subs.add(this.orches.dangerAlert$.subscribe(alert => {
      this.dangerAlert = alert;
    }));
    this.orches.connect();
  }
  ...
  dismissDangerAlert(): void {
    this.orches.dangerAlert$.next(null);
  }
